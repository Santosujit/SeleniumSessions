...
//search in Google : Selenium Java API Documentation
...
public class ElementUtil {
	 ...
	 * @param timeOut
	 */
	public void actionsClickWhenReady(By locator, int timeOut) {// action class click as sometimes normal click is not
																// working
		WebDriverWait wait = new WebDriverWait(driver, timeOut);
		WebElement element = wait.until(ExpectedConditions.elementToBeClickable(locator));// elementToBeClickable method
																							// is used
		Actions act = new Actions(driver);// create Actions class object
		act.moveToElement(element).click().build().perform();
	}

	public void clickWhenReady(By locator, int timeOut) {// normal click
		WebDriverWait wait = new WebDriverWait(driver, timeOut);
	 ...
	 */
	// you have to keep checking or retry until the WebElement is available
	// dyanamic wait is 10 s- if element is found within 2sec, remaining 8 sec will
	// be ignored
	// It's an Interview Question

	public WebElement retryingElement(By locator) {// retryingElement() method will return a WebElement
		...
		while (attempts < 30) {
			try {
				...
				// keep checking whether element is found or not
				// If element is found break the while loop
				break;
			} catch (NoSuchElementException e) {
				// If the element is not found, wait for 500 ms, then attempt++
				try {
					// If the element is not found, wait for 500 ms, then attempt++
					Thread.sleep(500);
